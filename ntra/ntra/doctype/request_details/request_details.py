...
def check_availabilty(timeslot, meeting_rooms):
	# frappe.msgprint(f"{(len(timeslot.strftime('%Y-%m-%d %H:%M')))} {timeslot.strftime('%Y-%m-%d %H:%M:%S')}")
	# frappe.db.sql(f"SELECT COUNT(name) FROM `tabRequest Details` WHERE `available_slot` = '2025-01-12 09:00' AND meeting_rooms = 'Room !'")
	return frappe.db.count("Request Details", {"available_slot": timeslot.strftime('%Y-%m-%d %H:%M:%S'), 'meeting_rooms':meeting_rooms, 'docstatus':1}) < 1
...
def get_available_slots_between(query_start_time, query_end_time, settings):
	...
	return timeslots
class RequestDetails(Document):
	@frappe.whitelist(allow_guest=True)
	def get_meeting_room_details(self):
		meeting_room = frappe.get_cached_doc('Meetings Room', self.meeting_rooms)
		return meeting_room.amenities_details
		pass
	def validate(self):
		if self.guest_number > self.room_capacity:
			frappe.throw(_("Number of guests exceeds room capacity"))
	...
	def get_appointment_slots(self):
		...
		# Database queries
		settings = frappe.get_doc("Meetings Room", self.meeting_rooms)
		holiday_list = []
		if settings.holiday_list:
		...
		for timeslot in timeslots:
			...
			if _is_holiday(converted_timeslot.date(), holiday_list):
				...
				continue
			# Check availability
			if check_availabilty(timeslot, self.meeting_rooms) and converted_timeslot >= now:
	...
	pass
@frappe.whitelist(allow_guest=True)
def get_appointment_slots(date, meeting_rooms):
	...
	# Database queries
	settings = frappe.get_doc("Meetings Room", meeting_rooms)
	holiday_list = []
	if settings.holiday_list:
		holiday_list = frappe.get_doc("Holiday List", settings.holiday_list)
	timeslots = get_available_slots_between(query_start_time, query_end_time, settings)
	...
	for timeslot in timeslots:
		...
		if _is_holiday(converted_timeslot.date(), holiday_list):
			...
			continue
		# Check availability
		if check_availabilty(timeslot, meeting_rooms) and converted_timeslot >= now:
			converted_timeslots.append(dict(time=converted_timeslot, availability=True))
		else:
			converted_timeslots.append(dict(time=converted_timeslot, availability=False))
	date_required = datetime.datetime.strptime(date + " 00:00:00", format_string).date()
