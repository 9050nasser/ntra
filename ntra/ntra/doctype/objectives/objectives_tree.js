...
frappe.treeview_settings["Objective"] = {
	get_tree_nodes: "ntra.ntra.doctype.objective.objective.get_children",
	filters: [
		{
			fieldname: "company",
			fieldtype: "Select",
		...
		},
		{
			...
			get_query() {
				const company =
					frappe.treeview_settings["Objective"].page.fields_dict.company.get_value();
	...
	fields: [
		...
		},
		{
			...
			reqd: 1,
			default() {
				const treeview = frappe.treeview_settings["Objective"].treeview;
				let employee =
		...
		},
		{
			...
			description: __(
				"Link the cycle and tag KRA to your Objective to update the appraisal's Objective score based on the Objective progress",
			),
	...
	onload(treeview) {
		frappe.treeview_settings["Objective"].page = {};
		$.extend(frappe.treeview_settings["Objective"].page, treeview.page);
		treeview.make_tree();
	...
	onrender(node) {
		// show KRA against the Objective
		if (node.data.kra) {
	...
	post_render(treeview) {
		frappe.treeview_settings["Objective"].treeview = {};
		$.extend(frappe.treeview_settings["Objective"].treeview, treeview);
	},
	get_label(node) {
...
function update_progress(node, progress) {
	return frappe
		...
		.then((r) => {
			if (!r.exc && r.message) {
				frappe.treeview_settings["Objective"].treeview.tree.load_children(
					frappe.treeview_settings["Objective"].treeview.tree.root_node,
					true,
				);

				frappe.show_alert({
					message: __("Objective updated successfully"),
					indicator: "green",
				});
			} else {
				frappe.msgprint(__("Could not update Objective"));
			}
		});
}
