...
def get_children(doctype: str, parent: str, is_root: bool = False, **filters) -> list[dict]:
	...
	query = (
		...
		.select(
			Goal.name.as_("value"),
			Goal.objectives.as_("title"),
			Goal.is_group.as_("expandable"),
			Goal.status,
			Goal.employee,
			Goal.employee_name,
			Goal.objectives_cycle,
			Goal.progress,
			Goal.kra,
		)
	...
	if filters.get("goal"):
		query = query.where(Goal.parent_objectives == filters.get("goal"))
	elif parent and not is_root:
		# via expand child
		query = query.where(Goal.parent_objectives == parent)
	else:
		ifnull = CustomFunction("IFNULL", ["value", "default"])
		query = query.where(ifnull(Goal.parent_objectives, "") == "")
...
def _update_goal_completion_status(goals: list[dict]) -> list[dict]:
	for goal in goals:
		if goal.expandable:  # group node
			total_goals = frappe.db.count("Objectives", dict(parent_goal=goal.value))

			if total_goals:
				completed = frappe.db.count("Objectives", {"parent_objectives": goal.value, "status": "Completed"}) or 0
