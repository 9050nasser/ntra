...
frappe.treeview_settings["Goal"] = {
	get_tree_nodes: "ntra.overrides.goal.get_children",
	filters: [
		{
			fieldname: "company",
			fieldtype: "Select",
	...
	get_label(node) {
		if (node.title && node.title !== node.label) {
			return (
				__(node.title) + ` <span class="text-muted">(Department: ${node.data.custom_department}) (${node.data.employee_name})</span> <span style="color: red">(Weight: ${node.data.custom_weight}%) (Progress: ${node.data.custom_weight_progress}%) (Result: ${node.data.custom_weight_progress_result}%)</span>`
			);
		} else {
