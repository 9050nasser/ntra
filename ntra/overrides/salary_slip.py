...
def calculate_tax_by_tax_slab(annual_taxable_earning, tax_slab, eval_globals=None, eval_locals=None):
    ...
    for d in tax_slab.other_taxes_and_charges:
        if flt(d.min_taxable_income) and flt(d.min_taxable_income) > annual_taxable_earning:
            print(f"Skipping tax because the annual taxable earning is less than min taxable income {d.min_taxable_income}")
            continue

        if flt(d.max_taxable_income) and flt(d.max_taxable_income) < annual_taxable_earning:
            print(f"Skipping tax because the annual taxable earning is greater than max taxable income {d.max_taxable_income}")
            continue

        additional_tax = tax_amount * flt(d.percent) / 100
        print(f"Applying additional tax {additional_tax} for charges with percent {d.percent}")
        tax_amount += additional_tax

    print(f"Total calculated tax: {tax_amount}")
    return tax_amount
